...
> ⚠️ **Rankings under active review (as of 2026-05-22).** A suite-wide
> investigation surfaced two issues with the headline data:
> (1) ~80 problems across the 10-language suite use `solve()` functions that
> internally cache the answer after first computation — making warm bench
> iterations measure cache-return cost (nanoseconds) instead of real algorithm
> cost (seconds). The `max(time_ns, cold_start_ns)` metric partly compensates,
> but cross-language comparisons are still distorted because each language's
> cold-time can include JIT / tier-1 warm-up artifacts asymmetrically.
> (2) Algorithm choices vary across languages for the same problem
> (e.g., one language using a closed-form walker while another runs brute
> force), so the chart conflates language-runtime efficiency with
> algorithm-choice quality.
> A multi-session campaign is underway to strip caching from `solve()` and
> align algorithms; until it completes, treat the rank ordering below as
